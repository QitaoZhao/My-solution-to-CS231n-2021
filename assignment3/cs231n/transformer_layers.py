...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, query, key, value, attn_mask=None):
        ...
        N, T, D = value.shape
        # Create a placeholder, to be overwritten by your code below.
        # Note that its shape was originally written as (N, T, D), and there might be a mistake.
        output = torch.empty((N, S, D))
        ############################################################################
        ...
        query = query.reshape(N, S, H, D//H).permute(0, 2, 1, 3)
        key = key.reshape(N, T, H, D//H).permute(0, 2, 3, 1)
        value = value.reshape(N, T, H, D//H).permute(0, 2, 1, 3)
        Y = torch.matmul(query, key) / math.sqrt(D//H)   
        if attn_mask is not None:
            Y = Y.masked_fill(attn_mask==False, -float('inf'))
        Y = self.dropout(self.softmax(Y))
        Y = torch.matmul(Y, value).permute(0, 2, 1, 3).reshape(N, S, D)

...
def rnn_step_forward(x, prev_h, Wx, Wh, b):
    ...
    next_h = np.tanh(b + prev_h.dot(Wh) + x.dot(Wx))
    cache = {'x': x, 'prev_h': prev_h, 'next_h': next_h,
             'Wx': Wx, 'Wh': Wh}
...
def word_embedding_backward(dout, cache):
    ...
    # x provides indices, then add gradient from dout to corresponding elements of dW
    np.add.at(dW, x, dout)

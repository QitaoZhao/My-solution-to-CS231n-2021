...
class CaptioningRNN:
    ...
    def loss(self, features, captions):
        ...
        if self.cell_type == 'rnn':
            h, cache_rnn_forward = rnn_forward(x, h0, Wx, Wh, b)
            scores, cache_temporal_affine = temporal_affine_forward(h, W_vocab, b_vocab)
            loss, dscores = temporal_softmax_loss(scores, captions_out, mask) 

            # Backward_path
            dh, dW_cocab, db_vocab = temporal_affine_backward(dscores, cache_temporal_affine)
            dx, dh0, dWx, dWh, db = rnn_backward(dh, cache_rnn_forward)
            dW_embed = word_embedding_backward(dx, cache_word_embedding)
            _, dW_proj, db_proj = affine_backward(dh0, cache_affine)

            grads['W_vocab'], grads['b_vocab'] = dW_cocab, db_vocab
    ...
    def sample(self, features, max_length=30):
        ...
        x, _ = word_embedding_forward((captions_in + self._start).astype(np.int32), W_embed)
        ...
        if self.cell_type == 'rnn':
            for t in range(max_length):
                if t == 0:
                    next_h, _ = rnn_step_forward(x, h0.reshape(N, 1, -1), Wx, Wh, b)
                else:
                    next_h, _ = rnn_step_forward(x, next_h, Wx, Wh, b)
                scores, _ = temporal_affine_forward(next_h, W_vocab, b_vocab)
                captions_in[:, 0] = np.argmax(scores.reshape(N, -1), axis=1)
                x, _ = word_embedding_forward(captions_in.astype(np.int32), W_embed)
                captions[:, t] = captions_in[:, 0]
